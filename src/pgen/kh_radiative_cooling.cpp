...
//    - iprob=2 : tanh profile with white noise perturbation at interface
...
void ProblemGenerator::UserProblem(ParameterInput *pin, const bool restart) {
  ...
  KOKKOS_LAMBDA(int m, int k, int j, int i) {
    ...
    if (iprob == 1) {
      ...
      Real perturb = rand_gen.frand()-0.5; // random perturbation in the range [-0.5, 0.5]
      vy = -amp*2.0*vshear_delta*(perturb)*exp( -SQR((x2v - y_cold)/sigma) );
      vz = 0.0;
      scal = y0 - y1*tanh((x2v - y_cold)/a_char);
    ...
    par_for("kh_bcs", DevExeSpace(),0,(nmb-1),0,(n3-1),0,(n1-1),
      KOKKOS_LAMBDA(int m, int k, int i) {
        ...
        for (int j=0; j<ng; ++j) {
          ...
          w0_(m,IVY,k,js-j-1,i) = -w0_(m,IVY,k,js+j,i);
          w0_(m,IVZ,k,js-j-1,i) = -w0_(m,IVZ,k,js+j,i);
          for (int n=nfluid; n<(nfluid+nscalars); ++n) {
      ...
      if (mb_bcs.d_view(m,BoundaryFace::outer_x2)==BoundaryFlag::user) {
        for (int j=0; j<ng; ++j) {
          ...
          //w0_(m,IVY,k,je+j+1,i) = w0_(m,IVY,k,je,i)*w0_(m,IDN,k,je,i)/density_hot; //constant mass flux outflow
