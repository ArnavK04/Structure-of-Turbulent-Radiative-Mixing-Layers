...
void HistoryOutput::LoadHydroHistoryData(HistoryData *pdata, Mesh *pm) {
  ...
  Real temperature_code = pmbp->punit->temperature_cgs();
  ...
  if (eos_data.is_ideal) {
    pdata->nhist = 8+4;           //+4 for cold gas mass, hot gas mass, intermediate gas mass, Total radiative losses
  } else {
    pdata->nhist = 7;
  }
  ...
  // ADD LABELS FOR COLD GAS PROPERTIES
  pdata->label[nhydro_+3] = "Cold-Mass (<5e4 K)";
  pdata->label[nhydro_+4] = "Hot-Mass (>5e5 K)";
  pdata->label[nhydro_+5] = "Intermediate-Mass (5e4 < T < 5e5 K)";
  pdata->label[nhydro_+6] = "Edot_total";
  ...
  KOKKOS_LAMBDA(const int &idx, array_sum::GlobalSum &mb_sum) {
    ...
    // ADD COLD GAS PROPERTIES
    Real int_energy = (hvars.the_array[IEN] - hvars.the_array[nhydro_+1] - hvars.the_array[nhydro_+2] - hvars.the_array[nhydro_])/vol;
    Real temp_gas_code = int_energy*(eos_data.gamma - 1.0)/(u0_(m,IDN,k,j,i));
    Real temp_gas = temp_gas_code*temperature_code;
    Real lambda = ISMCoolFn(temp_gas)/cooling_unit;

    hvars.the_array[nhydro_+6] = vol*SQR(u0_(m,IDN,k,j,i))*lambda;

    if (temp_gas < 50000.0){
      hvars.the_array[nhydro_+3] = vol*u0_(m,IDN,k,j,i);
      hvars.the_array[nhydro_+4] = 0.0;
      hvars.the_array[nhydro_+5] = 0.0;
    } else if (temp_gas > 500000.0) {
      hvars.the_array[nhydro_+3] = 0.0;
      hvars.the_array[nhydro_+4] = vol*u0_(m,IDN,k,j,i);
      hvars.the_array[nhydro_+5] = 0.0;
    }
    else {
      hvars.the_array[nhydro_+3] = 0.0;
      hvars.the_array[nhydro_+4] = 0.0;
      hvars.the_array[nhydro_+5] = vol*u0_(m,IDN,k,j,i);
    }
